...
class PANEL_GameMaterial(Panel):
    ...
    def draw(self, context):
        ...
        if scene.D_gradientcolor== 'Presets':
           q_material_name = context.scene.materials_mats_metals
        else:
        ...
        if scene.D_gradientcolor== 'Presets':
           ...
           row.template_icon_view(scene, "materials_mats_metals", show_labels=True)
...
class GM_material_select(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        if q_mat[layer_idx].name == "Custom gradient":
           ...
           context.scene.materials_mats_metals = q_mat[layer_idx].name
...
class GM_generate_textures1(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        q_filepath = os.path.join(os.path.dirname( bpy.data.filepath  ), q_activeobj[:-4]+"albedo.tga")

        q_filepathnew = os.path.join(os.path.dirname( bpy.data.filepath  ), q_activeobj[:-4]+"albedo0001.tga")
