...
The script can convert multiple high poly into one low poly, you just have to select all the meshes you want to include in your low poly. Don't worry your high polys wont be touched, you will keep them separated with their names and modifiers.
...
Texture settings:
- Mask: this option will allow you later to apply the gradient on areas that have the same color baked. You must define the colors with the diffuse color of your model. Use saturated colors and and hue far from one another to make sure that the masks read correctly (two colors close will be interpreted as the same color mask).
